        ...
        for (const auto& entry : fs::directory_iterator{ folder }) {
          ...
          //  Mat filteredStats, filteredCentroids;
           ...
           // get_rectangle_for_detector(lab_channels[a], filteredStats, filteredCentroids, img);
            ...
            float lambda_S = 0.45, lambda_D = 0.35, lambda_U = 0.2;
            float tao_tb = 0.67;
            float tao_S = 0.45;
            int tao_v = 32;

            Mat lateral_stats = detector(lab_channels, img, lambda_S, lambda_D, lambda_U, tao_v, tao_S, tao_tb); 
            ...
            if (lateral_stats.rows < 2) {
               ...
               //  get_rectangle_for_detector(HSV_channels[B], filteredStatsHSV, filteredCentroidsHSV, img);
                ...
                lateral_stats_HSV = detector(lab_channels, img,  lambda_S, lambda_D, lambda_U, tao_v, tao_S, tao_tb); 
...
int main(int argc, char** argv)
{
  ...
  //  Mat image = imread("C:\\testdrive1583149188.5726807.png"); 
    ...
    Mat lateral_stats; //= detector(new_weight / 2, lab_channels, image);
    /*
        if (lateral_stats.rows < 2) {
            ...
            Mat stats_ = detector(filteredStatsHSV, filteredCentroidsHSV, new_weight / 2);
